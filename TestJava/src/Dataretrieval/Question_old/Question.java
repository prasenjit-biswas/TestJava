...
public class Question  {
	...
	public void setLocalRandoms(ArrayList<randomVariableUtil> localRandoms) {
	 ...
	 /* public static void getContentLinks(String inputXML ){
		  try{
		     Document theXML = (new SAXBuilder()).build(new StringReader(inputXML));
		     Element contentLinks = theXML.getRootElement();
		     ...
		     if( contentLinks != null && ("contentLinks").equals(contentLinks.getName()) ){
		    	...
		    	while(contentLinkItr.hasNext() ){
		    		...
		    		System.out.println(contentLinkEle.getChild("linkType").getText()); 
		    		System.out.println(contentLinkEle.getChild("linkPOLICY").getText()); 
		    	} 
	...
	public static void main(String argv[]) throws Exception{
		...
		try {
			...
			//Question ID for different types of Question
			//13117647083234300/13118421523870800 - MCQ            True/Flase - 13118577244722200
			//13118421523799800 - fill in the blank                Yes/No - 13118581862073700
			//13118421523830700 - section break                    Check All - 13118594570894600
			//Survey - 	13118601279855500						   Matching - 13118607334487300
			//Ranking - 13118604416686400						   Doc - Not able to find the Question ID
			
			getQuestion(con,"13252698236874063");//13252698001212363  119334241965773
			
			con.createClob();
			ResultSet rs = null;
			
			//getQuestionsFromTest(con, "13252698112462444");
			
			/*PreparedStatement pstmt1 = con.prepareStatement("update test_question_xref set maxpoints = ? where questionid ='1' and   testid = '1'");
			pstmt1.setString(1, null);
			//pstmt1.setNull(1, java.sql.Types.INTEGER);
			pstmt1.executeUpdate();*/
			
			/*PreparedStatement pstmt = con.prepareStatement("select maxpoints, points from test_question_xref where testid = 1 and questionid = 1 ");
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()){
				System.out.println(rs.getString("maxpoints"));
				System.out.println(rs.getInt("maxpoints"));
			}*/
			/*String query = "SELECT " +
			               "t.spec.extract('/question/selectionTitle/text()').getStringVal()  selectionTitle ," +
			               "t.spec.extract('/question/instructor_info/text()').getStringVal()  instructorInfo ," +
			               "t.spec.extract('/question/answerProperties/policy_palatte/text()').getStringVal()  policy_palatte_val " +
				  	       " FROM  abc t"; 
			*/
			/*String query = "SELECT  t.spec.extract('/question/contentLinks').getStringVal()  contentLinks"+
			               " FROM abc t WHERE  t.NAME = '11'"; 

			
			PreparedStatement pstmt = con.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()){
				System.out.println("contentLinks --> "+rs.getString("contentLinks"));
				getContentLinks(rs.getString("contentLinks"));
			}*/
			
			/*String query = "SELECT "+ 
			"xref.pickerxml.extract('/question/selectionTitle/text()').getStringVal() selectionTitle ,"+ 
			"xref.pickerxml.extract('/question/sourceInfo/text()').getStringVal() sourceInfo ,"+
			 "xref.pickerxml.extract('/question/displayType/text()').getStringVal() displayType ,"+
			  "xref.pickerxml.extract('/question/actualType/text()').getStringVal() actualType ,"+
			   "xref.pickerxml.extract('/question/localRandoms/text()').getStringVal() localRandoms ,"+
			    "xref.pickerxml.extract('/question/questionProperties/text()').getStringVal() qProperties ,"+ 
			    "xref.pickerxml.extract('/question/isChat/text()').getStringVal() isChat ,"+ 
			    "xref.pickerxml.extract('/question/isChemDraw/text()').getStringVal() isChemDraw ,"+ 
			    "xref.pickerxml.extract('/question/contentLinks').getStringVal() contentLinks"+ 
			     " FROM test_question_xref xref WHERE xref.questionid = '121061172422432' and xref.testid = '121061172421829'";
			
			PreparedStatement pstmt = con.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()){
				System.out.println(" selectionTitle "+rs.getString("selectionTitle"));
				System.out.println(" sourceInfo "+rs.getString("sourceInfo"));
				System.out.println(" displayType "+rs.getString("displayType"));
				System.out.println(" actualType "+rs.getString("actualType"));
				System.out.println(" localRandoms "+rs.getString("localRandoms"));
				System.out.println(" qProperties "+rs.getString("qProperties"));
				System.out.println(" isChat "+rs.getString("isChat"));
				System.out.println(" isChemDraw "+rs.getString("isChemDraw"));
				System.out.println(" contentLinks "+rs.getString("contentLinks"));
	...
	private static Question getQuestionInstance(int questionType)throws Exception{
		...
		//System.out.println("questionType : " + textQuestionType);
		return question;		
	}
	...
	public static Question questionFromStream(Question result,
		...
		//System.out.println(" Printing QTEXT ################"+result.getQtext());
		return( result );
	...
	private static void printData(Question question)throws Exception {
		...
		/*richMediaUtil rmu = question.getQuestionMedia();
		
		System.out.println("mediaString : " + rmu.mediaString);
		System.out.println("movieHeight : " + rmu.movieHeight);
		System.out.println("movieWidth : " + rmu.movieWidth);
		System.out.println("position : " + rmu.position);
		System.out.println("type : " + rmu.type);
		*/
	...
	throws Exception {
		...
		try {
			...
			richMediaUtil questionMedianew =null;
			if (hasMedia){
				questionMedianew= new richMediaUtil( theInput, format );				
			}
			question.setQuestionMedia(questionMedianew);
			// read choices
