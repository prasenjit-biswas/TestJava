...
public class worksheetQ extends Question {
	...
	public static worksheetQ getWorksheetQ(Question result,
		...
		worksheetQ worksheetQ = (worksheetQ) q.getQuestion(result, theInput, format);
		...
		for (int i = 0; i < count; i++) {
			worksheet_answer theAnswer = worksheet_answer
					.read(theInput, format);
			if (theAnswer != null) {
		...
		if (count != worksheetQ.getAnswers().size())
			System.out.println("count should be " + count + "; it actually is "
					+ worksheetQ.getAnswers().size());
