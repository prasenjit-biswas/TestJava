...
//        this.init();
    ...
    /**
     * Static function: <tt>getString(String s)</tt>
     * Return the MD5 fingerprint for string s in hex.
     */
    public static String getString (String s) {
	MD5 md = new MD5();
	md.addInput(s);
	byte [] fp = md.getMD();
	return( util.bytes2hexStr(fp) );
    }
    ...
    public static String md5stream (InputStream theStream)
	{
		try
		{
			byte [] buff = new byte[1024];
			MesgDigest md = new MD5();
			int nread;
			while ( (nread=theStream.read(buff, 0, 1024)) >= 0 )
				md.addInput(buff, nread);

			return util.bytes2hexStr(md.getMD());
		}
		catch (IOException e)
		{
			e.printStackTrace();
    ...
    public static void main(String args[]){
    	String MD5Key = MD5.getString("Madison Square Gardenstodo=repository_questionSetPull&ezid=13252698000283565&questionList=13252698000283568:13252698000021790");
